...
from src.backend.utils.schemas import NoteGetSchemaResponse, NotePostSchemaResponse
...
class ApiHelper:
    ...
    async def create_note(data: dict, session: AsyncSession) -> JSONResponse:
        ...
        content = NotePostSchemaResponse.model_validate({"note_id":id}).model_dump()
    ...
    async def get_note_by_id(id: int, session: AsyncSession) -> JSONResponse:
        ...
        return ApiHelper._success_response(
            status_code=200,
            content=validated_note.model_dump()
        )
    ...
    @staticmethod
    @handle_exceptions
    async def get_average_note_length(
    ...
    async def get_longest_notes(
        ...
        return ApiHelper._success_response(
            status_code=200,
            content=longest_notes
        )

    @staticmethod
    @handle_exceptions
    async def get_shortest_notes(
            top_n: int,
            session: AsyncSession
    ) -> JSONResponse:
